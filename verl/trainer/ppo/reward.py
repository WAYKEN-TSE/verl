...
def get_custom_reward_fn(config):
    ...
    if not file_path:#file_path == None
        return None
...
def load_reward_manager(config, tokenizer, num_examine, **reward_kwargs):
    ...
    Args:
        ...
        num_examine (int): 需要检查的样本数量。
        **reward_kwargs: 奖励函数的其它参数。

    Returns:
        object: 奖励管理器对象。

    Raises:
        NotImplementedError: 如果配置的奖励管理器不在支持列表中。
    ...
    if compute_score is None:
        sandbox_config = config.reward_model.get("sandbox_fusion")#none
    ...
    #final_compute_score:  <class 'function'>
    return reward_manager_cls(
        tokenizer=tokenizer,
        num_examine=num_examine,
        compute_score=final_compute_score,
        reward_fn_key=config.data.reward_fn_key,# data_source
        **reward_kwargs,
    )
